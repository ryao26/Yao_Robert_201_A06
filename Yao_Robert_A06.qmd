...
[https://github.com/ryao26/Yao_Robert_201_A06.git](https://github.com/ryao26/Yao_Robert_201_A06.git)
...
for (i in 1:length(player_info)){
  # assigns height to respective player
...
for (player in player_info){
  # calculates and appends ppg to list
  points_per_game <- append(points_per_game,
...
```{r}
# applies provided function and simplifies to vector
ppg_vec <- sapply(points_per_game, function(x) round(x, 2))
ppg_vec
```
...
```{r}
# print mat_rand for comparison
mat_rand

# create function
rounding <- function(int){
  # find number of digits in number
  int_digits <- nchar(as.character(int))
  # round based on number of digits
  int <- round(int, 1 - int_digits)
}

# applying function
mat_rand_rounded <- apply(mat_rand, 1:2, rounding)
# print new matrix for comparison
mat_rand_rounded
```
...
```{r}
# apply function on subset of only month columns
monthly_avg <- apply(max_temps[,3:14], 2, mean)
monthly_avg
```
...
```{r}
# calculating medians
colorado_medians <- apply(colorado_temps[,3:14], 1, median)

# calculating min/max
max_temp <- max(colorado_medians)
min_temp <- min(colorado_medians)

# years since 1895
max_year <- which.max(colorado_medians) + 1895
min_year <- which.min(colorado_medians) + 1895

paste0("The maximum median annual temperature of ", max_temp, " occurred in ",
  max_year, " and the minimum median annual temperature of ", min_temp,
  " occurred in ", min_year, ".")
```
...
sum_digits <- function(n){
  ...
  # recursive case returns last digit + function(truncated)
